...
export default {
    created() {
        ...
        bus.$on(ACTION_COMPONECT_SELECT, val => {
            this.$nextTick(() => {
                const getSelectRuipeng = Array.from(document.querySelectorAll('h4')).filter(el => el.innerText.toLowerCase() === val)
                if (getSelectRuipeng[0]) {
                    const ruipengTop = getSelectRuipeng[0].getBoundingClientRect().top
                    window.scrollTo(0, window.pageYOffset + ruipengTop - 20) // 20 for padding
                }
            })
        })
    },
