<template>
    <div class="rp-calendar">
        <div class="rp-calendar__header">
            <div class="rp-calendar__title">
                {{ i18nDate }}
            </div>
            <div class="rp-calendar__button-group" v-if="validatedRange.length === 0">
                <rp-button-group>
                    <rp-button type="plain" size="mini" @click="selectDate('prev-month')">
                        {{ t('el.datepicker.prevMonth') }}
                    </rp-button>
                    <rp-button type="plain" size="mini" @click="selectDate('today')">
                        {{ t('el.datepicker.today') }}
                    </rp-button>
                    <rp-button type="plain" size="mini" @click="selectDate('next-month')">
                        {{ t('el.datepicker.nextMonth') }}
                    </rp-button>
                </rp-button-group>
            </div>
        </div>
        <div class="rp-calendar__body" v-if="validatedRange.length === 0" key="no-range">
            <date-table :date="date" :selected-day="realSelectedDay" :first-day-of-week="realFirstDayOfWeek" @pick="pickDay" />
        </div>
        <div v-else class="rp-calendar__body" key="has-range">
            <date-table
...
import fecha from 'ruipeng-ui/src/utils/date'
import RpButton from 'ruipeng-ui/packages/button'
import RpButtonGroup from 'ruipeng-ui/packages/button-group'
import DateTable from './date-table'
import { validateRangeInOneMonth } from 'ruipeng-ui/src/utils/date-util'
        ...
        range: {
            ...
            validator(range) {
                if (Array.isArray(range)) {
                    return (
                        range.length === 2 && range.every(item => typeof item === 'string' || typeof item === 'number' || item instanceof Date)
                    )
                } else {
                    return true
                }
