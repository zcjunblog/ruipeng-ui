<template>
    <div :class="['rp-cascader-panel', border && 'is-bordered']" @keydown="handleKeyDown">
...
import { noop, coerceTruthyValueToArray, isEqual, isEmpty, valueEquals } from 'ruipeng-ui/src/utils/util'
...
const getSibling = (el, distance) => {
    const { parentNode } = el
    if (parentNode) {
        const siblings = parentNode.querySelectorAll('.rp-cascader-node[tabindex="-1"]')
        ...
        handleKeyDown(e) {
            ...
            switch (keyCode) {
                ...
                case KeyCode.left:
                    ...
                    if (preMenu) {
                        const expandedNode = preMenu.$el.querySelector('.rp-cascader-node[aria-expanded="true"]')
                ...
                case KeyCode.right:
                    ...
                    if (nextMenu) {
                        const firstNode = nextMenu.$el.querySelector('.rp-cascader-node[tabindex="-1"]')
                        focusNode(firstNode)
                    }
                    break
                case KeyCode.enter:
