<template>
    <div
        ref="reference"
        :class="['rp-cascader', realSize && `rp-cascader--${realSize}`, { 'is-disabled': isDisabled }]"
    ...
    >
        ...
        >
            <template slot="suffix">
                <i v-if="clearBtnVisible" key="clear" class="rp-input__icon rp-icon-circle-close" @click.stop="handleClear"></i>
                <i
                    v-else
                    key="arrow-down"
                    :class="['rp-input__icon', 'rp-icon-arrow-down', dropDownVisible && 'is-reverse']"
                    @click.stop="toggleDropDownVisible()"
                ></i>
            </template>
        </rp-input>

        <div v-if="multiple" class="rp-cascader__tags">
            <rp-tag
            ...
            <input
                ...
                type="text"
                class="rp-cascader__search-input"
        ...
        <transition name="rp-zoom-in-top" @after-leave="handleDropdownLeave">
            <div v-show="dropDownVisible" ref="popper" :class="['rp-popper', 'rp-cascader__dropdown', popperClass]">
                <rp-cascader-panel
                    ref="panel"
                    v-show="!filtering"
                ...
                <rp-scrollbar
                    ...
                    v-show="filtering"
                    tag="ul"
                    class="rp-cascader__suggestion-panel"
                    view-class="rp-cascader__suggestion-list"
                ...
                >
                    <template v-if="suggestions.length">
                        <li
                            ...
                            :key="item.uid"
                            :class="['rp-cascader__suggestion-item', item.checked && 'is-checked']"
                            :tabindex="-1"
                            @click="handleSuggestionClick(index)"
                        >
                            <span>{{ item.text }}</span>
                            <i v-if="item.checked" class="rp-icon-check"></i>
                        </li>
                    </template>
                    <slot v-else name="empty">
                        <li class="rp-cascader__empty-text">{{ t('el.cascader.noMatch') }}</li>
                    </slot>
                </rp-scrollbar>
            </div>
        </transition>
...
import RpCascaderPanel from 'ruipeng-ui/packages/cascader-panel'
import AriaUtils from 'ruipeng-ui/src/utils/aria-utils'
import { t } from 'ruipeng-ui/src/locale'
import { isEqual, isEmpty, kebabCase } from 'ruipeng-ui/src/utils/util'
import { isUndefined, isFunction } from 'ruipeng-ui/src/utils/types'
import { isDef } from 'ruipeng-ui/src/utils/shared'
...
export default {
    ...
    methods: {
        ...
        focusFirstNode() {
            this.$nextTick(() => {
                ...
                if (filtering && suggestionPanel) {
                    firstNode = suggestionPanel.$el.querySelector('.rp-cascader__suggestion-item')
                } else {
                    const firstMenu = popper.querySelector('.rp-cascader-menu')
                    firstNode = firstMenu.querySelector('.rp-cascader-node[tabindex="-1"]')
                }
        ...
        handleSuggestionKeyDown(event) {
            ...
            switch (keyCode) {
                case KeyCode.enter:
                    ...
                    break
                case KeyCode.up:
                    const prev = target.previousRpementSibling
                    prev && prev.focus()
                    break
                case KeyCode.down:
                    const next = target.nextRpementSibling
                    next && next.focus()
                    break
                case KeyCode.esc:
                case KeyCode.tab:
        ...
        updateStyle() {
            ...
            const inputInner = $el.querySelector('.rp-input__inner')

            if (!inputInner) return

            const tags = $el.querySelector('.rp-cascader__tags')
            let suggestionPanelRp = null

            if (suggestionPanel && (suggestionPanelRp = suggestionPanel.$el)) {
                const suggestionList = suggestionPanelRp.querySelector('.rp-cascader__suggestion-list')
