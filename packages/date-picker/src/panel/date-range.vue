<template>
    <transition name="rp-zoom-in-top" @after-leave="$emit('dodestroy')">
        <div
            v-show="visible"
            class="rp-picker-panel rp-date-range-picker rp-popper"
            :class="[
                {
        ...
        >
            <div class="rp-picker-panel__body-wrapper">
                <slot name="sidebar" class="rp-picker-panel__sidebar"></slot>
                <div class="rp-picker-panel__sidebar" v-if="shortcuts">
                    <button
                        type="button"
                        class="rp-picker-panel__shortcut"
                ...
                <div class="rp-picker-panel__body">
                    <div class="rp-date-range-picker__time-header" v-if="showTime">
                        <span class="rp-date-range-picker__editors-wrap">
                            <span class="rp-date-range-picker__time-picker-wrap">
                                <rp-input
                                    size="small"
                                    :disabled="rangeState.selecting"
                                    ref="minInput"
                                    :placeholder="t('el.datepicker.startDate')"
                                    class="rp-date-range-picker__editor"
                                    :value="minVisibleDate"
                                    @input="val => handleDateInput(val, 'min')"
                                    @change="val => handleDateChange(val, 'min')"
                                />
                            </span>
                            <span class="rp-date-range-picker__time-picker-wrap" v-clickoutside="handleMinTimeClose">
                                <rp-input
                                    size="small"
                                    class="rp-date-range-picker__editor"
                                    :disabled="rangeState.selecting"
                        ...
                        <span class="rp-icon-arrow-right"></span>
                        <span class="rp-date-range-picker__editors-wrap is-right">
                            <span class="rp-date-range-picker__time-picker-wrap">
                                <rp-input
                                    size="small"
                                    class="rp-date-range-picker__editor"
                            ...
                            <span class="rp-date-range-picker__time-picker-wrap" v-clickoutside="handleMaxTimeClose">
                                <rp-input
                                    size="small"
                                    class="rp-date-range-picker__editor"
                                    :disabled="rangeState.selecting"
                    ...
                    <div class="rp-picker-panel__content rp-date-range-picker__content is-left">
                        <div class="rp-date-range-picker__header">
                            <button type="button" @click="leftPrevYear" class="rp-picker-panel__icon-btn rp-icon-d-arrow-left"></button>
                            <button type="button" @click="leftPrevMonth" class="rp-picker-panel__icon-btn rp-icon-arrow-left"></button>
                            <button
                                type="button"
                                @click="leftNextYear"
                                v-if="unlinkPanels"
                                ...
                                class="rp-picker-panel__icon-btn rp-icon-d-arrow-right"
                            ></button>
                            <button
                                type="button"
                                @click="leftNextMonth"
                                ...
                                class="rp-picker-panel__icon-btn rp-icon-arrow-right"
                            ></button>
                            <div>{{ leftLabel }}</div>
                        </div>
                        <date-table
                    ...
                    <div class="rp-picker-panel__content rp-date-range-picker__content is-right">
                        <div class="rp-date-range-picker__header">
                            <button
                                type="button"
                                @click="rightPrevYear"
                                ...
                                :disabled="!enableYearArrow"
                                :class="{ 'is-disabled': !enableYearArrow }"
                                class="rp-picker-panel__icon-btn rp-icon-d-arrow-left"
                            ></button>
                            <button
                                type="button"
                                ...
                                :disabled="!enableMonthArrow"
                                :class="{ 'is-disabled': !enableMonthArrow }"
                                class="rp-picker-panel__icon-btn rp-icon-arrow-left"
                            ></button>
                            <button type="button" @click="rightNextYear" class="rp-picker-panel__icon-btn rp-icon-d-arrow-right"></button>
                            <button type="button" @click="rightNextMonth" class="rp-picker-panel__icon-btn rp-icon-arrow-right"></button>
                            <div>{{ rightLabel }}</div>
                        </div>
                        <date-table
                            selection-mode="range"
            ...
            <div class="rp-picker-panel__footer" v-if="showTime">
                <rp-button size="mini" type="text" class="rp-picker-panel__link-btn" @click="handleClear">
                    {{ t('el.datepicker.clear') }}
                </rp-button>
                <rp-button plain size="mini" class="rp-picker-panel__link-btn" :disabled="btnDisabled" @click="handleConfirm(false)">
...
import {
    ...
    extractDateFormat,
    extractTimeFormat
} from 'ruipeng-ui/src/utils/date-util'
...
export default {
    ...
    methods: {
        ...
        handleRangePick(val, close = true) {
            ...
            // workaround for https://github.com/RpemeFE/element/issues/7539, should remove this block when we don't have to care about Chromium 55 - 57
            setTimeout(() => {
                this.maxDate = maxDate
                this.minDate = minDate
            }, 10)
    ...
    components: { TimePicker, DateTable, RpInput, RpButton }
}
</script>
