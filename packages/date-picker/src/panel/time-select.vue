<template>
    <transition name="rp-zoom-in-top" @before-enter="handleMenuEnter" @after-leave="$emit('dodestroy')">
        <div ref="popper" v-show="visible" :style="{ width: width + 'px' }" :class="popperClass" class="rp-picker-panel time-select rp-popper">
            <rp-scrollbar noresize wrap-class="rp-picker-panel__content">
                <div
...
export default {
    ...
    methods: {
        ...
        scrollToOption(selector = '.selected') {
            const menu = this.$refs.popper.querySelector('.rp-picker-panel__content')
