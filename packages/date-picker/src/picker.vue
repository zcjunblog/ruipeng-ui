<template>
    <rp-input
        class="rp-date-editor"
        :class="'rp-date-editor--' + type"
        ...
        ref="reference"
    >
        <i slot="prefix" class="rp-input__icon" :class="triggerClass" @click="handleFocus"></i>
        <i slot="suffix" class="rp-input__icon" @click="handleClickIcon" :class="[showClose ? '' + clearIcon : '']" v-if="haveTrigger"></i>
    </rp-input>
    <div
        class="rp-date-editor rp-range-editor rp-input__inner"
        :class="[
            'rp-date-editor--' + type,
            pickerSize ? `rp-range-editor--${pickerSize}` : '',
        ...
        v-else
    >
        <i :class="['rp-input__icon', 'rp-range__icon', triggerClass]"></i>
        <input
            autocomplete="off"
            ...
            @focus="handleFocus"
            class="rp-range-input"
        />
        <slot name="range-separator">
            <span class="rp-range-separator">{{ rangeSeparator }}</span>
        </slot>
        <input
            autocomplete="off"
        ...
        <i
            ...
            class="rp-input__icon rp-range__close-icon"
        ></i>
    </div>
</template>
...
import { formatDate, parseDate, isDateObject, getWeekNumber } from 'ruipeng-ui/src/utils/date-util'
...
import merge from 'ruipeng-ui/src/utils/merge'
...
export default {
        ...
        pickerVisible(val) {
            ...
            if (val) {
                ...
                if (this.validateEvent) {
                    this.dispatch('RpFormItem', 'el.form.blur')
                }
                this.$emit('blur', this)
                this.blur()
            }
    ...
    computed: {
        ...
        triggerClass() {
            return this.prefixIcon || (this.type.indexOf('time') !== -1 ? 'rp-icon-time' : 'rp-icon-date')
        },
