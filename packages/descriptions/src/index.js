...
import { isFunction } from 'ruipeng-ui/src/utils/types'
...
export default {
    ...
    methods: {
        ...
        getRows() {
            const children = (this.$slots.default || []).filter(
                vnode => vnode.tag && vnode.componentOptions && vnode.componentOptions.Ctor.options.name === 'RpDescriptionsItem'
            )
    ...
    render() {
        ...
        return (
            <div class="rp-descriptions">
                {title || extra || $slots.title || $slots.extra ? (
                    <div class="rp-descriptions__header">
                        <div class="rp-descriptions__title">{$slots.title ? $slots.title : title}</div>
                        <div class="rp-descriptions__extra">{$slots.extra ? $slots.extra : extra}</div>
                    </div>
                ) : null}

                <div class="rp-descriptions__body">
                    <table
                        class={[
                            'rp-descriptions__table',
                            { 'is-bordered': border },
                            descriptionsSize ? `rp-descriptions--${descriptionsSize}` : ''
                        ]}
                    >
                        {rows.map(row => (
