...
export default {
    ...
    watch: {
        visible(val) {
            this.broadcast('RpDropdownMenu', 'visible', val)
            this.$emit('visible-change', val)
        },
        focusing(val) {
            const selfDefine = this.$el.querySelector('.rp-dropdown-selfdefine')
            if (selfDefine) {
                // 自定义
                if (val) {
        ...
        if (splitButton) {
            triggerRpm = (
                <rp-button-group>
                    ...
                    <rp-button ref="trigger" type={type} size={dropdownSize} class="rp-dropdown__caret-button" disabled={disabled}>
                        <i class="rp-dropdown__icon rp-icon-arrow-down" />
                    </rp-button>
                </rp-button-group>
            )
        } else {
