...
export default {
    ...
    computed: {
        ...
        validateIcon() {
            return {
                validating: 'rp-icon-loading',
                success: 'rp-icon-circle-check',
                error: 'rp-icon-circle-close'
            }[this.validateState]
        },
        textareaStyle() {
    ...
    methods: {
        ...
        calcIconOffset(place) {
            let elList = [].slice.call(this.$el.querySelectorAll(`.rp-input__${place}`) || [])
            ...
            if (this.$slots[pendant]) {
                el.style.transform = `translateX(${place === 'suffix' ? '-' : ''}${
                    this.$el.querySelector(`.rp-input-group__${pendant}`).offsetWidth
                }px)`
            } else {
