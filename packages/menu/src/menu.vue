...
import Menubar from 'ruipeng-ui/src/utils/menu/aria-menubar';
import { addClass, removeClass, hasClass } from 'ruipeng-ui/src/utils/dom';
...
export default {
  ...
  render (h) {
    const component = (
      <ul
        ...
        class={{
          'rp-menu--horizontal': this.mode === 'horizontal',
          'rp-menu--collapse': this.collapse,
          "rp-menu": true
        }}
      >
        { this.$slots.default }
      ...
      render(createRpement, context) {
        const data = {
          ...
          on: {
            ...
            enter(el) {
              addClass(el, 'rp-opacity-transition');
              el.style.opacity = 1;
            },

            afterEnter(el) {
              removeClass(el, 'rp-opacity-transition');
              el.style.opacity = '';
            },
            ...
            beforeLeave(el) {
              ...
              if (hasClass(el, 'rp-menu--collapse')) {
                removeClass(el, 'rp-menu--collapse');
                el.dataset.oldOverflow = el.style.overflow;
                el.dataset.scrollWidth = el.clientWidth;
                addClass(el, 'rp-menu--collapse');
              } else {
                addClass(el, 'rp-menu--collapse');
                el.dataset.oldOverflow = el.style.overflow;
                el.dataset.scrollWidth = el.clientWidth;
  ...
  watch: {
    ...
    collapse(value) {
      ...
      this.broadcast('RpSubmenu', 'toggle-collapse', value);
    }
  },
  methods: {
      ...
      if (this.router && hasIndex) {
        this.routeToItem(item, (error) => {
          ...
          if (error) {
            ...
            // https://github.com/RpemeFE/element/issues/17044
