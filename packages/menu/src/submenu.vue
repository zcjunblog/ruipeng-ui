...
export default {
    ...
    computed: {
        ...
        menuTransitionName() {
            return this.rootMenu.collapse ? 'rp-zoom-in-left' : 'rp-zoom-in-top'
        },
        opened() {
        ...
        isFirstLevel() {
            ...
            while (parent && parent !== this.rootMenu) {
                if (['RpSubmenu', 'RpMenuItemGroup'].indexOf(parent.$options.componentName) > -1) {
                    isFirstLevel = false
                    break
                } else {
    ...
    methods: {
        ...
        handleClick() {
            ...
            this.dispatch('RpMenu', 'submenu-click', this)
        },
        handleMouseenter(event, showTimeout = this.showTimeout) {
            ...
            this.dispatch('RpSubmenu', 'mouse-enter-child')
        ...
        handleMouseleave(deepDispatch = false) {
            ...
            this.dispatch('RpSubmenu', 'mouse-leave-child')
            clearTimeout(this.timeout)
            this.timeout = setTimeout(() => {
            ...
            if (this.appendToBody && deepDispatch) {
                if (this.$parent.$options.name === 'RpSubmenu') {
    ...
    render(h) {
        ...
        const popupMenu = (
            <transition name={menuTransitionName}>
                <div
                    ref="menu"
                    v-show={opened}
                    class={[`rp-menu--${mode}`, popperClass]}
                ...
                >
                    <ul
                        role="menu"
                        class={['rp-menu rp-menu--popup', `rp-menu--popup-${currentPlacement}`]}
        ...
        const inlineMenu = (
            <rp-collapse-transition>
                <ul role="menu" class="rp-menu rp-menu--inline" v-show={opened} style={{ backgroundColor: rootMenu.backgroundColor || '' }}>
                    {$slots.default}
                </ul>
            </rp-collapse-transition>
        )
        ...
        return (
            ...
            >
                ...
                >
                    {$slots.title}
                    <i class={['rp-submenu__icon-arrow', submenuTitleIcon]} />
                </div>
