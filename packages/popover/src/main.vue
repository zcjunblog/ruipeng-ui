<template>
    <span>
        <transition :name="transition" @after-enter="handleAfterEnter" @after-leave="handleAfterLeave">
            <div
                class="rp-popover rp-popper"
                :class="[popperClass, content && 'rp-popover--plain']"
                ref="popper"
            ...
            >
                <div class="rp-popover__title" v-if="title" v-text="title"></div>
                <slot>{{ content }}</slot>
            </div>
        </transition>
        <span class="rp-popover__reference-wrapper" ref="wrapper">
...
export default {
    ...
    mounted() {
        ...
        // 可访问性
        if (reference) {
            addClass(reference, 'rp-popover__reference')
        ...
        if (this.trigger === 'click') {
            ...
            if (this.tabindex < 0) {
                console.warn('[Rpement Warn][Popover]a negative taindex means that the element cannot be focused by tab key')
            }
