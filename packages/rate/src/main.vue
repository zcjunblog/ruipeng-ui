<template>
    ...
    >
        <span
            ...
            :key="key"
        >
            <i :class="[classes[item - 1], { hover: hoverIndex === item }]" class="rp-rate__icon" :style="getIconStyle(item)">
                <i v-if="showDecimalIcon(item)" :class="decimalIconClass" :style="decimalStyle" class="rp-rate__decimal"></i>
            </i>
        </span>
        <span v-if="showText || showScore" class="rp-rate__text" :style="{ color: textColor }">{{ text }}</span>
    </div>
</template>

<script>
import { hasClass } from 'ruipeng-ui/src/utils/dom'
import { isObject } from 'ruipeng-ui/src/utils/types'
...
export default {
    ...
    props: {
        ...
        iconClasses: {
            type: [Array, Object],
            default() {
                return ['rp-icon-star-on', 'rp-icon-star-on', 'rp-icon-star-on']
            }
        },
        voidIconClass: {
            type: String,
            default: 'rp-icon-star-off'
        },
        disabledVoidIconClass: {
            type: String,
            default: 'rp-icon-star-on'
        },
        disabled: {
            type: Boolean,
            default: false
    ...
    methods: {
        ...
        setCurrentValue(value, event) {
            ...
            if (this.allowHalf) {
                ...
                if (hasClass(target, 'rp-rate__item')) {
                    target = target.querySelector('.rp-rate__icon')
                }
                if (hasClass(target, 'rp-rate__decimal')) {
