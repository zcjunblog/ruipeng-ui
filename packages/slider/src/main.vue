<template>
    <div
        class="rp-slider"
        :class="{ 'is-vertical': vertical, 'rp-slider--with-input': showInput }"
        role="slider"
        :aria-valuemin="min"
        :aria-valuemax="max"
    ...
    >
        ...
        ></rp-input-number>
        <div
            class="rp-slider__runway"
            :class="{ 'show-input': showInput, disabled: sliderDisabled }"
            :style="runwayStyle"
            @click="onSliderClick"
            ref="slider"
        >
            <div class="rp-slider__bar" :style="barStyle"></div>
            ...
            <div class="rp-slider__stop" v-for="(item, key) in stops" :key="key" :style="getStopStyle(item)" v-if="showStops"></div>
            <template v-if="markList.length > 0">
                <div>
                    <div
                        v-for="(item, key) in markList"
                        :style="getStopStyle(item.position)"
                        class="rp-slider__stop rp-slider__marks-stop"
                        :key="key"
                    ></div>
                </div>
                <div class="rp-slider__marks">
                    <slider-marker
                        :mark="item.mark"
                        v-for="(item, key) in markList"
                        :key="key"
...
import RpInputNumber from 'ruipeng-ui/packages/input-number'
            ...
            if (this.range && Array.isArray(val)) {
                if (val[1] < this.min) {
                    ...
                    if (this.valueChanged()) {
                        this.dispatch('RpFormItem', 'el.form.change', [this.minValue, this.maxValue])
