...
export default {
    ...
    computed: {
        // 嵌入型的数据，watch 无法是检测到变化 https://github.com/RpemeFE/element/issues/14998
                    ...
                    lazyKeys.forEach(key => {
                        ...
                        if (rootLazyRowKeys.indexOf(key) !== -1) {
                            ...
                            if (newTreeData[key].children.length !== 0) {
                                throw new Error('[RpTable]children must be an empty array.')
                            }
            ...
            if (load && !rawTreeData[key].loaded) {
                ...
                load(row, treeNode, data => {
                    if (!Array.isArray(data)) {
                        throw new Error('[RpTable] data must be an array')
                    }
