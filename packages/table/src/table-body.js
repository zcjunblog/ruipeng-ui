import { arrayFindIndex } from 'ruipeng-ui/src/utils/util'
import { getCell, getColumnByCell, getRowIdentity } from './util'
import { getStyle, hasClass, removeClass, addClass } from 'ruipeng-ui/src/utils/dom'
import RpCheckbox from 'ruipeng-ui/packages/checkbox'
import RpTooltip from 'ruipeng-ui/packages/tooltip'
...
export default {
    ...
    render(h) {
        ...
        return (
            <table class="rp-table__body" cellspacing="0" cellpadding="0" border="0">
                <colgroup>
                ...
                <tbody>
                    ...
                    <rp-tooltip effect={this.table.tooltipEffect} placement="top" ref="tooltip" content={this.tooltipContent} />
                </tbody>
            </table>
        )
    },
    ...
    watch: {
        ...
        // update DOM manually. see https://github.com/RpemeFE/element/pull/13954/files#diff-9b450c00d0a9dec0ffad5a3176972e40
        'store.states.hoverRow'(newVal, oldVal) {
            ...
            raf(() => {
                const rows = this.$el.querySelectorAll('.rp-table__row')
    ...
    methods: {
        ...
        getRowClass(row, rowIndex) {
            const classes = ['rp-table__row']
            if (this.table.highlightCurrentRow && row === this.store.states.currentRow) {
                classes.push('current-row')
            }

            if (this.stripe && rowIndex % 2 === 1) {
                classes.push('rp-table__row--striped')
            }
        ...
        handleCellMouseEnter(event, row) {
            ...
            if (!(hasClass(cellChild, 'rp-tooltip') && cellChild.childNodes.length)) {
                return
            }
        ...
        rowRender(row, $index, treeRowData) {
            ...
            let display = true
            if (treeRowData) {
                rowClasses.push('rp-table__row--levrp-' + treeRowData.level)
                display = treeRowData.display
            }
            // 指令 v-show 会覆盖 row-style 中 display
            // 使用 :style 代替 v-show https://github.com/RpemeFE/element/issues/16995
