import Vue from 'vue'
import { hasClass, addClass, removeClass } from 'ruipeng-ui/src/utils/dom'
...
export default {
    ...
    render(h) {
        ...
        return (
            <table class="rp-table__header" cellspacing="0" cellpadding="0" border="0">
                <colgroup>
                ...
                <thead class={[{ 'is-group': isGroup, 'has-gutter': this.hasGutter }]}>
                    {this._l(columnRows, (columns, rowIndex) => (
                        <tr style={this.getHeaderRowStyle(rowIndex)} class={this.getHeaderRowClass(rowIndex)}>
                            {columns.map((column, cellIndex) => (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        {column.filterable ? (
                                            <span
                                                class="rp-table__column-filter-trigger"
                                                on-click={$event => this.handleFilterClick($event, column)}
                                            >
                                                <i class={['rp-icon-arrow-down', column.filterOpened ? 'rp-icon-arrow-up' : '']} />
                                            </span>
                                        ) : (
                                            ''
                                        )}
                            ...
                            {this.hasGutter ? <th class="rp-table__cell gutter" /> : ''}
                        </tr>
                    ))}
                </thead>
            </table>
    ...
    mounted() {
        // nextTick 是有必要的 https://github.com/RpemeFE/element/pull/11311
        this.$nextTick(() => {
    ...
    methods: {
        ...
        handleFilterClick(event, column) {
            ...
            cell = cell.querySelector('.rp-table__column-filter-trigger') || cell
        ...
        handleMouseDown(event, column) {
            ...
            if (this.draggingColumn && this.border) {
                ...
                const table = this.$parent
                const tableRp = table.$el
                const tableLeft = tableRp.getBoundingClientRect().left
                const columnRp = this.$el.querySelector(`th.${column.id}`)
                const columnRect = columnRp.getBoundingClientRect()
                const minLeft = columnRect.left - tableLeft + 30

                addClass(columnRp, 'noclick')
