...
class TableLayout {
    ...
    updateScrollY() {
        ...
        if (this.table.$el && bodyWrapper) {
            const body = bodyWrapper.querySelector('.rp-table__body')
    ...
    updateRpsHeight() {
        if (!this.table.$ready) return Vue.nextTick(() => this.updateRpsHeight())
        ...
        if (this.showHeader && !noneHeader && headerWrapper.offsetWidth > 0 && (this.table.columns || []).length > 0 && headerHeight < 2) {
            return Vue.nextTick(() => this.updateRpsHeight())
        }
