...
describe('Calendar', () => {
    ...
    it('create', async () => {
      ...
      `,
        ...
        const titleEl = vm.$el.querySelector('.rp-calendar__title')
        expect(/2019.*4/.test(titleEl.innerText)).to.be.true
        expect(vm.$el.querySelectorAll('thead th').length).to.equal(7)
        const rows = vm.$el.querySelectorAll('.rp-calendar-table__row')
    ...
    it('range', () => {
      ...
      `
        ...
        expect(vm.$el.querySelector('.rp-calendar__button-group')).to.be.a('null')
    })
    ...
    it('range tow monthes', async () => {
        vm = createVue(
            {
                template: `
      <rp-calendar :range="['2019-04-15', '2019-05-19']"></rp-calendar>
      `
            },
            true
        )
        const titleEl = vm.$el.querySelector('.rp-calendar__title')
        expect(/2019.*4/.test(titleEl.innerText)).to.be.true
        const dateTables = vm.$el.querySelectorAll('.rp-calendar-table.is-range')
    ...
    it('firstDayOfWeek', async () => {
        vm = createVue(
            {
                template: `
      <rp-calendar v-model="value" :first-day-of-week="0"></rp-calendar>
      `,
                data() {
                    return {
                        value: new Date('2019-04-01')
        ...
        const head = vm.$el.querySelector('.rp-calendar-table thead')
        expect(head.firstElementChild.innerText).to.be.equal('日')
        expect(head.lastElementChild.innerText).to.be.equal('六')
        const firstRow = vm.$el.querySelector('.rp-calendar-table__row')
    ...
    it('firstDayOfWeek in range mode', async () => {
        vm = createVue(
            {
                template: `
      <rp-calendar v-model="value" :first-day-of-week="7" :range="['2019-02-03', '2019-03-23']"></rp-calendar>
      `,
                data() {
                    return {
