...
const getValidOptions = (el, menuIndex) => getMenus(el)[menuIndex].querySelectorAll('.rp-cascader-node[tabindex="-1"]')
const getLabel = el => el.querySelector('.rp-cascader-node__label').textContent
...
describe('CascaderPanel', () => {
    ...
    it('expand and check', async () => {
        ...
        <rp-cascader-panel
          ref="panel"
          v-model="value"
          :options="options"></rp-cascader-panel>
      `,
                data() {
                    return {
                        value: [],
    ...
    it('with default value', async () => {
      ...
      `,
        ...
        expect(getOptions(el, 2)[0].querySelector('.rp-icon-check')).to.exist
    })
    ...
    it('multiple mode', async () => {
      ...
      `,
        ...
        const el = vm.$el
        const checkbox = getOptions(el, 0)[0].querySelector('.rp-checkbox')
        expect(checkbox).to.exist
        expect(checkbox.querySelector('.rp-checkbox__input').className).to.not.includes('is-checked')
        checkbox.querySelector('input').click()

        await waitImmediate()
        expect(checkbox.querySelector('.rp-checkbox__input').className).to.includes('is-checked')
    ...
    it('multiple mode with disabled default value', async () => {
      ...
      `,
        ...
        await waitImmediate()
        expect(checkbox).to.exist
        expect(checkbox.querySelector('.rp-checkbox__input').className).to.includes('is-indeterminate')
        checkbox.querySelector('input').click()
    ...
    it('check strictly in single mode', async () => {
      ...
      `,
        ...
        const el = vm.$el
        const radio = getOptions(el, 0)[0].querySelector('.rp-radio')
        ...
        expect(getOptions(el, 0)[1].querySelector('.rp-radio').className).to.includes('is-disabled')
    })
    ...
    it('check strictly in multiple mode', async () => {
      ...
      `,
        ...
        getOptions(el, 0)[0].click()
        await waitImmediate()
        expect(getOptions(el, 1)[0].querySelector('.rp-checkbox').className).to.not.includes('is-checked')
        getOptions(el, 1)[0]
            .querySelector('input')
            .click()
        await waitImmediate()
        expect(vm.value).to.deep.equal(['zhejiang', 'hangzhou'])
        expect(getOptions(el, 0)[1].querySelector('.rp-checkbox').className).to.includes('is-disabled')
    })
    ...
    it('dynamic loading', async () => {
      ...
      `,
        ...
        firstOption.click()
        await waitImmediate()
        expect(firstOption.querySelector('i').className).to.includes('rp-icon-loading')
        await wait(1000)
        expect(firstOption.querySelector('i').className).to.includes('rp-icon-arrow-right')
        expect(getMenus(el).length).to.equal(2)
        getOptions(el, 1)[0].click()
        await wait(1000)
        getOptions(el, 2)[0].click()
