...
const getMenus = el => el.querySelectorAll('.rp-cascader-menu')
const getOptions = (el, menuIndex) => getMenus(el)[menuIndex].querySelectorAll('.rp-cascader-node')
const selectedValue = ['zhejiang', 'hangzhou', 'xihu']
const getCloseButton = el => el.querySelectorAll('i.rp-tag__close')
...
describe('Cascader', () => {
    ...
    it('expand and check', async () => {
      ...
      `,
        ...
        expect(checkedOption.querySelector('i.rp-icon-check')).to.exist
    ...
    it('with default value', async () => {
      ...
      `,
        ...
        expect(getOptions(el, 2)[0].querySelector('i').className).to.includes('rp-icon-check')
        expect(vm.$el.querySelector('input').value).to.equal('Zhejiang / Hangzhou / West Lake')
    })
    ...
    it('clearable', async () => {
      ...
      `,
        ...
        await waitImmediate()
        const closeBtn = vm.$el.querySelector('i.rp-input__icon')
    ...
    it('show last level label', async () => {
        ...
        <rp-cascader
          v-model="value"
          :options="options"
          :show-all-levels="false"></rp-cascader>
      `,
                data() {
                    return {
                        value: selectedValue,
    ...
    it('multiple mode', async () => {
      ...
      `,
        ...
        getOptions(document.body, 0)[0]
            .querySelector('.rp-checkbox input')
            .click()
        await waitImmediate()
        expect(vm.value.length).to.equal(3)
        expect(getCloseButton(vm.$el).length).to.equal(3)

        const tags = vm.$el.querySelectorAll('.rp-tag')
        const closeBtn = tags[0].querySelector('.rp-tag__close')
        expect(tags.length).to.equal(3)
        expect(closeBtn).to.exist
        closeBtn.click()
        await waitImmediate()
        expect(vm.value.length).to.equal(2)
        expect(vm.$el.querySelectorAll('.rp-tag').length).to.equal(2)
    ...
    it('clearable in multiple mode', async () => {
      ...
      `,
        ...
        await waitImmediate()
        expect(getOptions(document.body, 0)[0].querySelector('.rp-checkbox.is-checked')).to.exist
        triggerEvent(vm.$el, 'mouseenter')
        await waitImmediate()
        const closeBtn = vm.$el.querySelector('i.rp-input__icon')
        expect(closeBtn).to.exist
    ...
    it('collapse tags', async () => {
      ...
      `,
        ...
        expect(tags[0].querySelector('.rp-tag__close')).to.exist
        expect(tags[1].querySelector('.rp-tag__close')).to.be.null
        tags[0].querySelector('.rp-tag__close').click()
        expect(tags[1].textContent).to.equal('+ 3')
        await waitImmediate()
        expect(vm.value.length).to.equal(3)
        vm.$el.querySelector('.rp-tag .rp-tag__close').click()
        await waitImmediate()
        vm.$el.querySelector('.rp-tag .rp-tag__close').click()
        await waitImmediate()
        expect(vm.$el.querySelector('.rp-tag')).to.exist
        // disabled tag can not be closed
        expect(vm.$el.querySelector('.rp-tag .rp-tag__close')).to.be.null
    })
    ...
    it('filterable', async () => {
      ...
      `,
        ...
        await wait(300)
        expect(body.querySelector('.rp-cascader__suggestion-list')).to.exist
        expect(body.querySelectorAll('.rp-cascader__suggestion-item').length).to.equal(3)
        body.querySelectorAll('.rp-cascader__suggestion-item')[0].click()
        await waitImmediate()
        expect(vm.value).to.deep.equal(selectedValue)
    })
    ...
    it('filter method', async () => {
      ...
      `,
        ...
        triggerEvent(input, 'input')
        await wait(300)
        expect(body.querySelector('.rp-cascader__suggestion-item').textContent).to.equal('Zhejiang / Hangzhou / West Lake')
    })
})
