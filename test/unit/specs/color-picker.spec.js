...
describe('ColorPicker', () => {
    ...
    afterEach(() => {
        vm.$destroy(true)
        destroyVM(vm)
        const dropdown = document.querySelector('.rp-color-dropdown')
    ...
    it('should show alpha slider when show-alpha=true', done => {
      ...
      `,
        ...
        const trigger = vm.$el.querySelector('.rp-color-picker__trigger')
        trigger.click()

        setTimeout(() => {
            const alphaSlider = document.querySelector('.rp-color-alpha-slider')
    ...
    it('should pick a color when confirm button click', done => {
      ...
      `,
        ...
        setTimeout(() => {
            const dropdown = document.querySelector('.rp-color-dropdown__btn')
            dropdown.click()
            vm.$nextTick(() => {
    ...
    it('should show correct rgb value', done => {
      ...
      `,
        ...
        setTimeout(() => {
            const input = document.querySelector('.rp-color-dropdown__value input')
    ...
    it('should init the right color when open', done => {
      ...
      `,
        ...
        setTimeout(() => {
            ...
            dropdown.click()
            vm.$nextTick(() => {
                const hueBar = document.querySelector('.rp-color-hue-slider__thumb')
    ...
    it('should clear a color when clear button click', done => {
      ...
      `,
        ...
        setTimeout(() => {
            const clearBtn = document.querySelector('.rp-color-dropdown__link-btn')
            clearBtn.click()
            setTimeout(() => {
    ...
    it('should change hue when clicking the hue bar', done => {
      ...
      `,
        ...
        setTimeout(() => {
            const hueBar = document.querySelector('.rp-color-hue-slider')
    ...
    it('should change hue when saturation is zero', done => {
      ...
      `,
        ...
        setTimeout(() => {
            ...
            hueBar.__vue__.handleClick({ target: null, clientX: 0, clientY: 1000 })
            vm.$nextTick(() => {
                const thumb = document.querySelector('.rp-color-hue-slider__thumb')
    ...
    it('should change alpha when clicking the alpha bar', done => {
      ...
      `,
        ...
        setTimeout(() => {
            const alphaBar = document.querySelector('.rp-color-alpha-slider')
    ...
    it('should change saturation and value when clicking the sv-panel', done => {
        const vm = createVue(
            {
                template: `
        <rp-color-picker v-model="color" color-format="hsv"></rp-color-picker>
      `,
        ...
        setTimeout(() => {
            const svPanel = document.querySelector('.rp-color-svpanel')
    ...
    it('should change color to the selected color', done => {
        const vm = createVue(
            {
                template: `
        <rp-color-picker v-model="color" show-alpha :predefine="colors"></rp-color-picker>
      `,
                data() {
                    return {
        ...
        setTimeout(() => {
            expect(document.querySelectorAll('.rp-color-predefine__color-selector').length === 9).to.be.true
            const selector = document.querySelector('.rp-color-predefine__color-selector:nth-child(4)')
            selector.click()
            vm.$nextTick(() => {
                const picker = vm.$children[0]
                ...
                const selector2 = document.querySelector('.rp-color-predefine__color-selector:nth-child(3)')
                selector2.click()
