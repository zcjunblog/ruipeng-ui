...
describe('Drawer', () => {
    ...
    it('create', async () => {
        vm = createVue(
            {
                template: `
        <rp-drawer :title='title' :visible='visible'></rp-drawer>
      `,
                data() {
                    return {
                        title,
        ...
        expect(vm.$el.querySelector('.rp-drawer__header').textContent).to.equal(title)
    ...
    it('render correct content', async () => {
        vm = createVue(
            {
                template: `
          <rp-drawer :title='title' :visible='visible'>
            <span>这是一段信息</span>
      ...
      `,
        ...
        await waitImmediate()
        expect(vm.$el.querySelector('.rp-drawer__body span').textContent).to.equal('这是一段信息')
        const footerBtns = vm.$el.querySelectorAll('.rp-button')
    ...
    it('should append to body, when append-to-body flag is true', async () => {
        vm = createVue(
            {
                template: `
        <rp-drawer :title='title' :visible='visible' :append-to-body='true'>
           <span> content </span>
        </rp-drawer>
      `,
                data() {
    ...
    it('should open and close drawer properly', async () => {
        vm = createVue({
            template: `
        <rp-drawer :title='title' :visible='visible' ref='drawer'>
           <span>${content}</span>
        </rp-drawer>
      `,
            data() {
    ...
    it('should destroy every child after drawer was closed when destroy-on-close flag is true', async () => {
        vm = createVue({
            template: `
        <rp-drawer :title='title' :visible='visible' :append-to-body='true' :destroy-on-close='true' ref='drawer'>
           <span>${content}</span>
        </rp-drawer>
      `,
            data() {
        ...
        await waitImmediate()
        expect(vm.$el.querySelector('.rp-drawer__body span').textContent).to.equal(content)
        vm.$refs.drawer.closeDrawer()
        await wait(400)
        expect(vm.$el.querySelector('.rp-drawer__body')).not.to.exist
    })
    ...
    it('should close dialog by clicking the close button', async () => {
        vm = createVue({
            template: `
        <rp-drawer :title='title' :visible.sync='visible' :append-to-body='true' :destroy-on-close='true' ref='drawer'>
           <span>${content}</span>
        </rp-drawer>
      `,
            data() {
        ...
        await waitImmediate()
        vm.$children[0].$el.querySelector('.rp-drawer__close-btn').click()
    ...
    it('should not show close button when show-close flag is false', async () => {
        vm = createVue({
            template: `
        <rp-drawer :title='title' :visible='visible' ref='drawer' :show-close='false'>
           <span>${content}</span>
        </rp-drawer>
      `,
            data() {
        ...
        expect(vm.$el.querySelector('.rp-drawer__close-btn')).not.to.exist
    })
    ...
    it('should have custom classes when custom classes were given', async () => {
        ...
        vm = createVue({
            template: `
        <rp-drawer :title='title' :visible='visible' ref='drawer' custom-class='${classes}'>
           <span>${content}</span>
        </rp-drawer>
      `,
            data() {
    ...
    it('should not render header when withHeader attribute is false', () => {
        vm = createVue({
            template: `
        <rp-drawer :title='title' :visible='visible' ref='drawer' :with-header='false'>
           <span>${content}</span>
        </rp-drawer>
      `,
            data() {
        ...
        expect(vm.$el.querySelector('.rp-drawer__header')).to.not.exist
    })
    ...
    describe('directions', () => {
        const renderer = direction => {
            return createVue({
                template: `
         <rp-drawer :title='title' :visible='visible' direction='${direction}'>
            <span>${content}</span>
         </rp-drawer>
        `,
                data: {
    ...
    describe('size', () => {
        const renderer = (size, isVertical) =>
            createVue({
                template: `
         <rp-drawer :title='title' :visible='visible' direction='${isVertical ? 'ltr' : 'ttb'}' size='${size}'>
            <span>${content}</span>
         </rp-drawer>
      `,
                data: {
        ...
        it('should effect height when drawer is vertical', async () => {
            ...
            expect(vm.$el.querySelector('.rp-drawer').style.width).to.equal('50%')
        })

        it('should effect width when drawer is horizontal', async () => {
            const size = '50%'
            vm = renderer(size, false)
            expect(vm.$el.querySelector('.rp-drawer').style.height).to.equal('50%')
        })
    })
})
