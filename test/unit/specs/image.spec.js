...
describe('Image', () => {
    ...
    it('create', async () => {
        ...
        const placeholder = vm.$el.querySelector('.rp-image__placeholder')
        const error = vm.$el.querySelector('.rp-image__error')
        let img = vm.$el.querySelector('.rp-image__inner')
        expect(placeholder).to.exist
        expect(error).to.be.null
        expect(img).to.be.null

        await wait()
        img = vm.$el.querySelector('.rp-image__inner')
    ...
    it('$attrs', async () => {
      ...
      `,
        ...
        await wait()
        const $img = vm.$el.querySelector('.rp-image__inner')
    ...
    it('pass event listeners', async () => {
        ...
        vm = createVue(
            {
                template: `
        <rp-image @click="handleClick" :src="src"></rp-image>
      `,
                data() {
                    return {
                        src
        ...
        await wait()
        vm.$el.querySelector('.rp-image__inner').click()
        await wait()
    ...
    it('big image preview', async () => {
        vm = createVue(
            {
                template: `
        <rp-image :src="src" :preview-src-list="srcList"></rp-image>
      `,
                data() {
                    return {
                        src: src,
        ...
        expect(document.querySelector('.rp-image-viewer__wrapper')).to.exist
        document.querySelector('.rp-image-viewer__close').click()
        await wait(1000)
        expect(document.querySelector('.rp-image-viewer__wrapper')).to.not.exist
    })
})
