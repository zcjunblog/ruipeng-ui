...
describe('Input', () => {
    ...
    it('suffixIcon', () => {
        vm = createVue(
            {
                template: `
        <rp-input suffix-icon="time"></rp-input>
      `
            },
            true
        )
        var icon = vm.$el.querySelector('.rp-input__icon')
    ...
    it('prefixIcon', () => {
        vm = createVue(
            {
                template: `
        <rp-input prefix-icon="time"></rp-input>
      `
            },
            true
        )
    ...
    it('size', () => {
      ...
      `
        ...
        expect(vm.$el.classList.contains('rp-input--large')).to.true
    })
    ...
    it('type', () => {
      ...
      `
        ...
        expect(vm.$el.classList.contains('rp-textarea')).to.true
    })
    ...
    it('rows', () => {
        vm = createVue(
            {
                template: `
        <rp-input type="textarea" :rows="3">
        </rp-input>
      `
            },
            true
        )
        expect(vm.$el.querySelector('.rp-textarea__inner').getAttribute('rows')).to.be.equal('3')
    })
        ...
        vm = createVue(
            {
                template: `
        <div>
          <rp-input type="textarea" :resize="resize"></rp-input>
        </div>
      `,
                data: {
                    resize: 'none'
        ...
        await waitImmediate()
        expect(vm.$el.querySelector('.rp-textarea__inner').style.resize).to.be.equal(vm.resize)
    ...
    it('Input contains Select and append slot', async () => {
        vm = createVue(
            {
                template: `
      <rp-input v-model="value" clearable class="input-with-select" ref="input">
      ...
      `,
        ...
        await wait()
        const suffixEl = document.querySelector('.input-with-select > .rp-input__suffix')
    ...
    it('validateEvent', async () => {
        ...
        vm = createVue(
            {
                template: `
        <rp-form :model="model" :rules="rules">
          <rp-form-item prop="input">
            <rp-input v-model="model.input" :validate-event="false">
            </rp-input>
          </rp-form-item>
        </rp-form>
      `,
    ...
    describe('Input Events', () => {
        ...
        `,
            ...
            await waitImmediate()
            vm.$el.querySelector('.rp-input__clear').click()
            await waitImmediate()
    ...
    it('sets value on textarea / input type change', async () => {
        vm = createVue(
            {
                template: `
        <rp-input :type="type" v-model="val" />
      `,
                data() {
                    return {
                        type: 'text',
    ...
    it('limit input and show word count', async () => {
      ...
      `,
        ...
        expect(inputElm1.querySelectorAll('.rp-input__count').length).to.equal(0)
        expect(inputElm2.querySelectorAll('.rp-input__count').length).to.equal(1)
        expect(inputElm3.querySelectorAll('.rp-input__count').length).to.equal(0)
        expect(inputElm4.classList.contains('is-exceed')).to.true

        vm.show = true
        await waitImmediate()
        expect(inputElm1.querySelectorAll('.rp-input__count').length).to.equal(1)
