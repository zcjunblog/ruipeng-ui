import VuePopup from 'ruipeng-ui/src/utils/popup'
...
describe('Mixin:vue-popup', () => {
    ...
    it('lock scroll', done => {
        vm = createTest(Popup, { modal: true })
        vm.open()
        expect(document.body.classList.contains('rp-popup-parent--hidden')).to.be.true
        vm.close()
        destroyVM(vm)
        setTimeout(() => {
            vm = createTest(Popup, { modal: true, lockScroll: false })
            vm.open()
            expect(document.body.classList.contains('rp-popup-parent--hidden')).to.be.false
            done()
        }, 200)
    })
