...
describe('Switch', () => {
    ...
    it('create', () => {
        ...
        const core = vm.$el.querySelector('.rp-switch__core')
        expect(core.style.backgroundColor).to.equal('rgb(255, 0, 0)')
        expect(core.style.width).to.equal('100px')
        expect(vm.$el.querySelector('.rp-switch__label--left').querySelector('span').textContent).to.equal('off')
    })

    it('switch with icons', () => {
        vm = createTest(Switch, {
            activeIconClass: 'rp-icon-check',
            inactiveIconClass: 'rp-icon-close'
        })

        const icon = vm.$el.querySelector('.rp-switch__label--left').querySelector('i')
        ...
        vm = createVue(
            {
                template: `
        <div>
          <rp-switch disabled v-model="value"></rp-switch>
        </div>
      `,
        ...
        vm.$el.querySelector('.rp-switch__core').click()
        Vue.nextTick(() => {
        ...
        vm = createVue(
            {
                template: `
        <div>
          <rp-switch v-model="value" :active-value="onValue" :inactive-value="offValue"></rp-switch>
        </div>
      `,
                data() {
                    return {
        ...
        vm = createVue(
            {
                template: `
        <div>
          <rp-switch :value="true"></rp-switch>
        </div>
      `
            },
            true
