...
describe('Table', () => {
    describe('rendering data is correct', () => {
        const vm = createVue({
            template: `
        <rp-table :data="testData">
          <rp-table-column prop="id" />
          <rp-table-column prop="name" label="片名" />
          <rp-table-column prop="release" label="发行日期" />
          <rp-table-column prop="director" label="导演" />
          <rp-table-column prop="runtime" label="时长（分）" />
        </rp-table>
      `,
        ...
        it('row length', () => {
            expect(vm.$el.querySelectorAll('.rp-table__body-wrapper tbody tr')).to.length(getTestData().length)
        })
    ...
    describe('attributes', () => {
        ...
        it('stripe', done => {
            ...
            setTimeout(_ => {
                expect(vm.$el.classList.contains('rp-table--striped')).to.true
                destroyVM(vm)
                done()
            }, DELAY)
        })
        ...
        it('border', done => {
            ...
            setTimeout(_ => {
                expect(vm.$el.classList.contains('rp-table--border')).to.true
                destroyVM(vm)
                done()
            }, DELAY)
        })
        ...
        it('fit', done => {
            ...
            setTimeout(_ => {
                expect(vm.$el.classList.contains('rp-table--fit')).to.false
                destroyVM(vm)
                done()
            }, DELAY)
        })
        ...
        it('show-header', done => {
            ...
            setTimeout(_ => {
                expect(vm.$el.querySelectorAll('.rp-table__header-wrapper').length).to.equal(0)
                destroyVM(vm)
                done()
            }, DELAY)
        })
        ...
        it('tableRowStyle[Object]', done => {
            ...
            setTimeout(_ => {
                expect(vm.$el.querySelector('.rp-table__body tr').style.height).to.equal('60px')
                destroyVM(vm)
                done()
            }, DELAY)
        })
        ...
        it('tableRowStyle[Function]', done => {
            ...
            setTimeout(_ => {
                let child1 = vm.$el.querySelector('.rp-table__body tr:nth-child(1)')
                let child2 = vm.$el.querySelector('.rp-table__body tr:nth-child(2)')
        ...
        it('current-row-key', done => {
            const vm = createVue(
                {
                    template: `
        <rp-table :data="testData" row-key="id" highlight-current-row :current-row-key="currentRowKey">
          <rp-table-column prop="name" label="片名" />
          <rp-table-column prop="release" label="发行日期" />
          <rp-table-column prop="director" label="导演" />
          <rp-table-column prop="runtime" label="时长（分）" />
      ...
      `,
            ...
            setTimeout(_ => {
                vm.currentRowKey = 1
                const tr = vm.$el.querySelector('.rp-table__body-wrapper tbody tr')
                setTimeout(_ => {
                    expect(tr.classList.contains('current-row')).to.be.true
                    vm.currentRowKey = 2

                    const rows = vm.$el.querySelectorAll('.rp-table__body-wrapper tbody tr')
                    setTimeout(_ => {
        ...
        it('select-on-indeterminate', done => {
            const vm = createVue(
                {
                    template: `
          <rp-table :data="testData" @selection-change="change" :select-on-indeterminate="false" ref="table">
            <rp-table-column type="selection" />
            <rp-table-column prop="name" label="name" />
            <rp-table-column prop="release" label="release" />
            <rp-table-column prop="director" label="director" />
            <rp-table-column prop="runtime" label="runtime" />
          </rp-table>
        `,
            ...
            setTimeout(_ => {
                vm.$el.querySelector('.rp-checkbox').click()
                setTimeout(_ => {
    ...
    describe('filter', () => {
        ...
        beforeEach(done => {
            vm = createVue(
                {
                    template: `
          <rp-table ref="table" :data="testData" @filter-change="handleFilterChange">
            <rp-table-column prop="name" label="片名" />
            <rp-table-column prop="release" label="发行日期" />
            <rp-table-column
              prop="director"
        ...
        it('render', () => {
            expect(vm.$el.querySelector('.rp-table__column-filter-trigger')).to.exist
        })

        it('click dropdown', done => {
            const btn = vm.$el.querySelector('.rp-table__column-filter-trigger')
            triggerEvent(btn, 'click', true, false)
            setTimeout(_ => {
                const filter = document.body.querySelector('.rp-table-filter')
        ...
        it('click filter', done => {
            ...
            setTimeout(_ => {
                ...
                // John Lasseter
                triggerEvent(filter.querySelector('.rp-checkbox'), 'click', true, false)
                // confrim button
                setTimeout(_ => {
                    triggerEvent(filter.querySelector('.rp-table-filter__bottom button'), 'click', true, false)
                    setTimeout(_ => {
                        expect(vm.filters['director']).to.be.eql(['John Lasseter'])
                        expect(vm.$el.querySelectorAll('.rp-table__body-wrapper tbody tr')).to.length(3)
                        document.body.removeChild(filter)
                        done()
                    }, DELAY)
                }, 100)
        ...
        it('click reset', done => {
            ...
            setTimeout(_ => {
                ...
                setTimeout(_ => {
                    // reset button
                    triggerEvent(filter.querySelectorAll('.rp-table-filter__bottom button')[1], 'click', true, false)
                    setTimeout(_ => {
                        expect(vm.filters['director']).to.be.eql([])
                        expect(filter.querySelector('.rp-table-filter__bottom button').classList.contains('is-disabled')).to.true
                        document.body.removeChild(filter)
                        destroyVM(vm)
                        done()
                    }, DELAY)
    ...
    describe('events', () => {
        const createTable = function(prop = '', opts) {
            return createVue(
                {
                    template: `
          <rp-table :data="testData" @${prop}="handleEvent">
            <rp-table-column type="selection" />
            <rp-table-column prop="name" />
            <rp-table-column prop="release" />
            <rp-table-column prop="director" />
            <rp-table-column prop="runtime"/>
          </rp-table>
        `,
        ...
        it('select', done => {
            ...
            setTimeout(_ => {
                vm.$el.querySelectorAll('.rp-checkbox')[1].click()
        ...
        it('cell-mouse-enter', done => {
            ...
            setTimeout(_ => {
                const cell = vm.$el.querySelectorAll('.rp-table__body .cell')[2] // first row
        ...
        it('cell-mouse-leave', done => {
            ...
            setTimeout(_ => {
                const cell = vm.$el.querySelectorAll('.rp-table__body .cell')[7] // second row
                const cell2 = vm.$el.querySelectorAll('.rp-table__body .cell')[2] // first row
        ...
        it('header-click', done => {
            ...
            setTimeout(_ => {
                const cell = vm.$el.querySelectorAll('.rp-table__header th')[1] // header[prop='name']
        ...
        it('sort-change', async () => {
            const vm = createVue({
                template: `
          <rp-table ref="table" :data="testData" :default-sort = "{prop: 'runtime', order: 'ascending'}">
            <rp-table-column prop="name" />
            <rp-table-column prop="release" />
            <rp-table-column prop="director" />
            <rp-table-column prop="runtime" sortable/>
          </rp-table>
        `,
    ...
    describe('column attributes', () => {
        const createTable = function(props1, props2, props3, props4, opts, tableProps) {
            return createVue(
                Object.assign(
                    {
                        template: `
          <rp-table :data="testData" ${tableProps || ''}>
            <rp-table-column prop="name" ${props1 || ''} />
            <rp-table-column prop="release" ${props2 || ''} />
            <rp-table-column prop="director" ${props3 || ''} />
            <rp-table-column prop="runtime" ${props4 || ''} />
          </rp-table>
        `,
        ...
        it('width', done => {
            ...
            setTimeout(_ => {
                const ths = toArray(vm.$el.querySelectorAll('.rp-table__header-wrapper col'))
                    .map(node => node.width)
                    .filter(o => o)
        ...
        it('fixed', done => {
            ...
            setTimeout(_ => {
                expect(toArray(vm.$el.querySelectorAll('.rp-table__fixed th:not(.is-hidden)')).map(node => node.textContent)).to.eql([
                    'test1',
                    'test3'
                ])

                expect(toArray(vm.$el.querySelectorAll('.rp-table__fixed-right th:not(.is-hidden)')).map(node => node.textContent)).to.eql([
                    'test2'
                ])
                expect(vm.$el.querySelector('.rp-table__body-wrapper').style.height).to.equal('')
                destroyVM(vm)
                done()
            }, DELAY)
        })
        ...
        it('formatter', done => {
            ...
            setTimeout(_ => {
                const cells = toArray(vm.$el.querySelectorAll('.rp-table__body-wrapper tbody tr td:first-child'))
        ...
        it('show-overflow-tooltip', done => {
            ...
            setTimeout(_ => {
                expect(vm.$el.querySelectorAll('.rp-tooltip')).to.length(5)
                destroyVM(vm)
                done()
            }, DELAY)
        })
        ...
        `,
            ...
            setTimeout(_ => {
                const headerCell = vm.$el.querySelector('.rp-table__header-wrapper thead tr th:first-child .cell')
        ...
        it('selectable', done => {
            const vm = createVue(
                {
                    template: `
          <rp-table :data="testData" @selection-change="change">
            <rp-table-column type="selection" :selectable="filterSelect" />
            <rp-table-column prop="name" label="name" />
            <rp-table-column prop="release" label="release" />
            <rp-table-column prop="director" label="director" />
            <rp-table-column prop="runtime" label="runtime" />
        ...
        `,
            ...
            setTimeout(_ => {
                expect(vm.$el.querySelector('.rp-checkbox').__vue__.checked).to.be.false
                setTimeout(_ => {
                    expect(vm.selected).to.length(0)
                    destroyVM(vm)
                    done()
        ...
        describe('type', () => {
          ...
          `,
            ...
            describe('= selection', () => {
                ...
                it('render', done => {
                    setTimeout(_ => {
                        expect(vm.$el.querySelectorAll('.rp-checkbox')).to.length(getTestData().length + 1)
                        done()
                    }, DELAY)
                })
                ...
                it('select one', done => {
                    ...
                    setTimeout(_ => {
                        vm2.$el.querySelectorAll('.rp-checkbox')[1].click()
            ...
            describe('= index', () => {
                ...
                it('render', done => {
                    setTimeout(_ => {
                        expect(
                            toArray(vm.$el.querySelectorAll('.rp-table__body-wrapper tbody tr td:first-child')).map(node => node.textContent)
                        ).to.eql(['1', '2', '3', '4', '5'])
                        destroyVM(vm)
                        done()
                    }, DELAY)
            ...
            describe('= expand', () => {
                const createInstance = function(extra) {
                    ...
                    return createVue(
                        {
                            template: `
            <rp-table row-key="id" :data="testData" @expand-change="handleExpand" ${extra}>
          ...
          `,
                ...
                it('works', done => {
                    ...
                    setTimeout(_ => {
                        expect(vm.$el.querySelectorAll('td.rp-table__expand-column').length).to.equal(5)
                        destroyVM(vm)
                        done()
                    }, DELAY)
                })
                ...
                it('should expand when click icon', done => {
                    const vm = createInstance()
                    setTimeout(_ => {
                        vm.$el.querySelector('td.rp-table__expand-column .rp-table__expand-icon').click()
                        setTimeout(_ => {
                            expect(vm.$el.querySelectorAll('.rp-table__expanded-cell').length).to.equal(1)
                            expect(vm.expandCount).to.equal(1)
                            vm.$el.querySelector('td.rp-table__expand-column .rp-table__expand-icon').click()
                            setTimeout(_ => {
                                expect(vm.$el.querySelectorAll('.rp-table__expanded-cell').length).to.equal(0)
                ...
                it('should set expanded rows using expandRowKeys', done => {
                    ...
                    setTimeout(_ => {
                        ...
                        setTimeout(_ => {
                            expect(vm.$el.querySelectorAll('.rp-table__expanded-cell').length).to.equal(2)
                            vm.expandRowKeys = [2]
                            setTimeout(_ => {
                                expect(vm.$el.querySelectorAll('.rp-table__expanded-cell').length).to.equal(1)
                                destroyVM(vm)
                ...
                it('should default-expand-all when default-expand-all is true', done => {
                    ...
                    setTimeout(_ => {
                        expect(vm.$el.querySelectorAll('.rp-table__expanded-cell').length).to.equal(5)
                        destroyVM(vm)
                        done()
                    }, DELAY)
                })
        ...
        describe('sortable', () => {
            ...
            it('sortable orders', done => {
                ...
                setTimeout(_ => {
                    ...
                    setTimeout(_ => {
                        const lastCells = vm.$el.querySelectorAll('.rp-table__body-wrapper tbody tr td:last-child')
    ...
    describe('summary row', () => {
        it('should render', done => {
            const vm = createVue(
                {
                    template: `
          <rp-table :data="testData" show-summary>
            <rp-table-column prop="name" />
            <rp-table-column prop="release"/>
            <rp-table-column prop="director"/>
            <rp-table-column prop="runtime"/>
        ...
        `,
            ...
            setTimeout(_ => {
                const footer = vm.$el.querySelector('.rp-table__footer')
        ...
        it('custom sum text', done => {
            const vm = createVue(
                {
                    template: `
          <rp-table :data="testData" show-summary sum-text="Time">
            <rp-table-column prop="name" />
            <rp-table-column prop="release"/>
            <rp-table-column prop="director"/>
            <rp-table-column prop="runtime"/>
        ...
        `,
            ...
            setTimeout(_ => {
                const cells = toArray(vm.$el.querySelectorAll('.rp-table__footer .cell'))
        ...
        it('custom summary method', done => {
            const vm = createVue(
                {
                    template: `
          <rp-table :data="testData" show-summary :summary-method="getSummary">
            <rp-table-column prop="name" />
            <rp-table-column prop="release"/>
            <rp-table-column prop="director"/>
            <rp-table-column prop="runtime"/>
    ...
    describe('multi level column', () => {
        ...
        `,
            ...
            setTimeout(_ => {
                const trs = vm.$el.querySelectorAll('.rp-table__header tr')
        ...
        it('should works', done => {
          ...
          <rp-table :data="testData">
            <rp-table-column prop="name" />
            <rp-table-column label="group">
              <rp-table-column label="group's group">
                <rp-table-column prop="release" />
                <rp-table-column prop="runtime"/>
              </rp-table-column>
              <rp-table-column prop="director" />
    ...
    describe('dynamic column attribtes', () => {
            ...
            const vm = createVue(
                {
                    template: `
          <rp-table :data="testData">
            <rp-table-column prop="name" :label="label"/>
            <rp-table-column prop="release" />
            <rp-table-column prop="director" />
            <rp-table-column prop="runtime" />
          </rp-table>
        `,
            ...
            setTimeout(() => {
                expect(vm.$el.querySelector('.rp-table__header th .cell').textContent).to.equal('name')
                vm.label = 'NAME'
                vm.$nextTick(() => {
                    expect(vm.$el.querySelector('.rp-table__header th .cell').textContent).to.equal('NAME')
                    destroyVM(vm)
            ...
            const vm = createVue(
                {
                    template: `
          <rp-table :data="testData">
            <rp-table-column prop="name" :align="align"/>
          </rp-table>
        `,
            ...
            setTimeout(() => {
                expect(vm.$el.querySelectorAll('.rp-table__body td.is-right').length === 0).to.be.true
                vm.align = 'right'
                vm.$nextTick(() => {
                    expect(vm.$el.querySelectorAll('.rp-table__body td.is-right').length > 0).to.be.true
                    destroyVM(vm)
            ...
            const vm = createVue(
                {
                    template: `
           <rp-table :data="testData">
            <rp-table-column prop="name" :align="align" :header-align="headerAlign"/>
          </rp-table>
        `,
            ...
            vm.$nextTick(() => {
                expect(vm.$el.querySelectorAll('.rp-table__header th.is-left').length).to.above(0)
                expect(vm.$el.querySelectorAll('.rp-table__header th.is-center').length).to.equal(0)
                expect(vm.$el.querySelectorAll('.rp-table__header th.is-right').length).to.equal(0)
                vm.align = 'right'
                vm.$nextTick(() => {
                    expect(vm.$el.querySelectorAll('.rp-table__header th.is-left').length).to.equal(0)
                    expect(vm.$el.querySelectorAll('.rp-table__header th.is-center').length).to.equal(0)
                    expect(vm.$el.querySelectorAll('.rp-table__header th.is-right').length).to.above(0)
                    vm.headerAlign = 'center'
                    vm.$nextTick(() => {
                        expect(vm.$el.querySelectorAll('.rp-table__header th.is-left').length).to.equal(0)
                        expect(vm.$el.querySelectorAll('.rp-table__header th.is-center').length).to.above(0)
                        expect(vm.$el.querySelectorAll('.rp-table__header th.is-right').length).to.equal(0)
                        vm.headerAlign = null
                        vm.$nextTick(() => {
                            expect(vm.$el.querySelectorAll('.rp-table__header th.is-left').length).to.equal(0)
        ...
        it('width', done => {
            const vm = createVue(
                {
                    template: `
          <rp-table :data="testData" :fit="false">
            <rp-table-column prop="name" :width="width"/>
          </rp-table>
        `,
            ...
            setTimeout(() => {
                expect(vm.$el.querySelector('.rp-table__body col').getAttribute('width')).to.equal('100')
                vm.width = 200
                setTimeout(() => {
                    expect(vm.$el.querySelector('.rp-table__body col').getAttribute('width')).to.equal('200')
                    destroyVM(vm)
                    done()
                }, 100)
            }, DELAY)
            ...
            const vm = createVue(
                {
                    template: `
          <rp-table :data="testData" :fit="false">
            <rp-table-column prop="name" :min-width="width"/>
          </rp-table>
        `,
        ...
        `,
            ...
            setTimeout(() => {
                expect(!vm.$el.querySelector('.rp-table__fixed')).to.be.true
                vm.fixed = true
                setTimeout(() => {
                    expect(!!vm.$el.querySelector('.rp-table__fixed')).to.be.true
                    destroyVM(vm)
        ...
        `,
            ...
            setTimeout(() => {
                let firstColumnContent = vm.$el.querySelector('.rp-table__body td .cell').textContent
                let secondColumnContent = vm.$el.querySelector('.rp-table__body td:nth-child(2) .cell').textContent
                expect(firstColumnContent !== secondColumnContent).to.be.true
                vm.prop = 'release'
                setTimeout(() => {
                    firstColumnContent = vm.$el.querySelector('.rp-table__body td .cell').textContent
                    secondColumnContent = vm.$el.querySelector('.rp-table__body td:nth-child(2) .cell').textContent
                    expect(firstColumnContent === secondColumnContent).to.be.true
                    destroyVM(vm)
                    done()
                }, 100)
    ...
    describe('methods', () => {
        const createTable = function(prop = '', opts) {
            return createVue(
                {
                    template: `
          <rp-table ref="table" :data="testData" @${prop}="handleEvent">
            <rp-table-column type="selection" />
            <rp-table-column prop="name" />
            <rp-table-column prop="release" />
            <rp-table-column prop="director" />
            ...
            const vm = createVue(
                {
                    template: `
        <div>
          <rp-table ref="table" :data="testData" @selection-change="change">
            <rp-table-column type="selection" />
            <rp-table-column prop="name" />
          </rp-table>
          <rp-table ref="table1" :data="testData1" @selection-change="change">
            <rp-table-column type="selection" />
            <rp-table-column prop="name" />
          </rp-table>
        </div>
        ...
        it('sort correct change icon', async () => {
            ...
            const vm = createVue({
                template: `
          <rp-table ref="table" :data="testData" >
            <rp-table-column prop="name" sortable />
            <rp-table-column prop="release" sortable />
            <rp-table-column prop="director" sortable />
            <rp-table-column prop="runtime" sortable />
          </rp-table>
        `,
                data() {
        ...
        it('setCurrentRow', async () => {
            const vm = createVue({
                template: `
        <div>
          <rp-table ref="table" :data="testData" highlight-current-row>
            <rp-table-column prop="name" sortable />
            <rp-table-column prop="release" sortable />
            <rp-table-column prop="director" sortable />
            <rp-table-column prop="runtime" sortable />
        ...
        `,
            ...
            await waitImmediate()
            const secondRow = vm.$el.querySelectorAll('.rp-table__row')[1]
        ...
        const vm = createVue(
            {
                template: `
        <rp-table :data="testData">
          <rp-table-column prop="name" label="片名" fixed />
          <rp-table-column prop="release" label="发行日期" />
          <rp-table-column prop="director" label="导演" />
          <rp-table-column prop="runtime" label="时长（分）" />
        </rp-table>
    ...
    it('highlight-current-row', done => {
        const vm = createVue(
            {
                template: `
        <rp-table :data="testData" highlight-current-row>
          <rp-table-column prop="name" label="片名" />
          <rp-table-column prop="release" label="发行日期" />
          <rp-table-column prop="director" label="导演" />
          <rp-table-column prop="runtime" label="时长（分）" sortable />
        </rp-table>
      `,
        ...
        setTimeout(_ => {
            ...
            setTimeout(_ => {
                ...
                setTimeout(_ => {
                    ...
                    const ths = vm.$el.querySelectorAll('.rp-table__header-wrapper thead th')
    ...
    it('keep highlight row when data change', done => {
        const vm = createVue(
            {
                template: `
        <rp-table :data="testData" highlight-current-row row-key="release">
          <rp-table-column prop="name" label="片名" />
          <rp-table-column prop="release" label="发行日期" />
          <rp-table-column prop="director" label="导演" />
          <rp-table-column prop="runtime" label="时长（分）" sortable />
      ...
      `,
        ...
        setTimeout(() => {
            let rows = vm.$el.querySelectorAll('.rp-table__body-wrapper tbody tr')
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    rows = vm.$el.querySelectorAll('.rp-table__body-wrapper tbody tr')
                    expect(rows[3].classList.contains('current-row')).to.be.true
                    destroyVM(vm)
                    done()
                }, DELAY)
    ...
    it('keep highlight row after sort', done => {
        const vm = createVue(
            {
                template: `
        <rp-table :data="testData" row-key="release" highlight-current-row >
          <rp-table-column prop="name" label="片名" />
          <rp-table-column prop="release" label="发行日期" />
          <rp-table-column prop="director" label="导演" />
          <rp-table-column prop="runtime" label="时长（分）" sortable />
      ...
      `,
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                expect(rows[1].classList.contains('current-row')).to.be.true
                const cells = vm.$el.querySelectorAll('.rp-table__header-wrapper thead th > .cell')
    ...
    it('table append is visible in viewport if height is 100%', async () => {
      ...
      `
        ...
        await waitImmediate()
        const emptyBlockEl = vm.$el.querySelector('.rp-table__empty-block')
    ...
    describe('tree', () => {
        ...
        it('render tree structual data', async () => {
            vm = createVue(
                {
                    template: `
          <rp-table :data="testData" row-key="release">
            <rp-table-column prop="name" label="片名" />
            <rp-table-column prop="release" label="发行日期" />
            <rp-table-column prop="director" label="导演" />
            <rp-table-column prop="runtime" label="时长（分）" />
        ...
        `,
            ...
            const rows = vm.$el.querySelectorAll('.rp-table__row')
            expect(rows.length).to.equal(7)
            const childRows = vm.$el.querySelectorAll('.rp-table__row--levrp-1')
            expect(childRows.length).to.equal(2)
            childRows.forEach(item => {
                expect(item.style.display).to.equal('none')
            })
            vm.$el.querySelector('.rp-table__expand-icon').click()
        ...
        it('load substree row data', async () => {
            vm = createVue(
                {
                    template: `
          <rp-table :data="testData" row-key="release" lazy :load="load">
            <rp-table-column prop="name" label="片名" />
            <rp-table-column prop="release" label="发行日期" />
            <rp-table-column prop="director" label="导演" />
            <rp-table-column prop="runtime" label="时长（分）" />
        ...
        `,
            ...
            const expandIcon = vm.$el.querySelector('.rp-table__expand-icon')
            expandIcon.click()

            await waitImmediate()

            expect(expandIcon.classList.contains('rp-table__expand-icon--expanded')).to.be.true
            expect(vm.$el.querySelectorAll('.rp-table__row').length).to.equal(8)
        })
        ...
        it('expand-row-keys & toggleRowExpansion', async () => {
            vm = createVue(
                {
                    template: `
          <rp-table :data="testData" row-key="release" lazy :load="load" :expand-row-keys="['2003-5-30']" ref="table">
            <rp-table-column prop="name" label="片名" />
            <rp-table-column prop="release" label="发行日期" />
            <rp-table-column prop="director" label="导演" />
            <rp-table-column prop="runtime" label="时长（分）" />
        ...
        `,
            ...
            const expandIcon = childRows[0].querySelector('.rp-table__expand-icon')
            expandIcon.click()
            ...
            await waitImmediate()
            expect(expandIcon.classList.contains('rp-table__expand-icon--expanded')).to.be.false
        })
    })
})
