...
describe('Tooltip', () => {
    ...
    it('create', done => {
        vm = createVue(`
      <rp-tooltip ref="tooltip" content="提示文字">
        <button>click</button>
      </rp-tooltip>`)
    ...
    it('custom popper class', done => {
        vm = createVue(`
      <rp-tooltip ref="tooltip" content="提示文字" popper-class="custom-popper">
        <button>click</button>
      </rp-tooltip>`)
        vm.$nextTick(_ => {
    ...
    describe('manual', () => {
        const vm = createVue(
            {
                template: `
        <rp-tooltip ref="tooltip" manual content="abc" v-model="show">
          <button>click</button>
        </rp-tooltip>
      `,
    ...
    it('light mode', done => {
        vm = createVue(`
      <rp-tooltip ref="tooltip" content="abc" effect="light">
        <button>abc</button>
      </rp-tooltip>
    `)
        vm.$nextTick(_ => {
    ...
    it('hide after', done => {
        vm = createVue(`
      <rp-tooltip ref="tooltip" content="提示文字" :hide-after="300">
        <button>click</button>
      </rp-tooltip>`)
        const tooltip = vm.$refs.tooltip
        vm.$nextTick(_ => {
    ...
    it('custom tabindex', () => {
        vm = createVue(
            `
      <rp-tooltip ref="tooltip" content="提示文字" :tabindex="-1">
        <button>click</button>
      </rp-tooltip>
    `,
            true
