...
describe('Transfer', () => {
    ...
    const createTransfer = (props, opts) => {
        return createVue(
            Object.assign(
                {
                    template: `
        <rp-transfer :data="testData" ref="transfer" ${props}>
        </rp-transfer>
      `,
    ...
    it('filterable', done => {
        ...
        const leftList = transfer.$el.querySelector('.rp-transfer-panel').__vue__
        leftList.query = '1'
        setTimeout(_ => {
    ...
    it('customize', () => {
        ...
        const label = transfer.querySelector('.rp-transfer-panel__header .rp-checkbox__label')
        expect(label.innerText.indexOf('表1') > -1).to.true
        expect(transfer.querySelector('.rp-transfer-panel__list .rp-checkbox__label span').innerText).to.equal('1 - 备选项 1')
    ...
    it('check', () => {
        ...
        const leftList = vm.$refs.transfer.$el.querySelector('.rp-transfer-panel').__vue__
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    const targetItems = [].slice.call(
                        vm.$el.querySelectorAll('.rp-transfer__buttons + .rp-transfer-panel .rp-transfer-panel__body .rp-checkbox__label span')
                    )
